...
#include<complex.h> //Numeros complexos-lfftw
...
const double sig=2.0;                  //sigma (nm)
const double phi=90.0;                   // Angulo de k (Graus)
const double E0=0.549;                   //Energy (eV)
const int eta=0;                        // vale K ou K'
...
for(i=0;i<nx;i++)
for(j=0;j<ny;j++){
A_x[i][j]=eta*(hc*beta/(4*l))*(pow(f_x.arr[i][j],2)-pow(f_y.arr[i][j],2)); // eV*s/nm
A_y[i][j]=-eta*(hc*beta/(4*l))*2*((f_x.arr[i][j])*(f_y.arr[i][j]));
A_z[i][j]=0.0;
}
...
for(i=0;i<nx;i++)
for(j=0;j<ny;j++)
fprintf(plot,"%0.3f \t %0.3f \t %0.3f\n",X[i],Y[j],(1/pow(10,-18))*B[i][j]);

fclose(plot);

system("python plot.py plotgraf pseudo_potential_vector.dat  x(nm) y(nm) B(T)");
